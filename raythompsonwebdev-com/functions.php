...
function raythompsonwebdev_filter_wp_title( $title, $sep ) {
    ...
    if ( $paged >= 2 || $page >= 2 )
        $title = "$title $sep " . sprintf( __( 'Page %s', 'raythompsonwebdev-com' ), max( $paged, $page ) );
 ...
 */
function raythompsonwebdev_scripts_own()
{
 ...
 * @return void
 */
function raythompsonwebdev_mapcontainer()
{
    ?>
 ...
 * @return void
 */
function raythompsonwebdev_google_script()
{
    ?>
...
<?php }
add_action('wp_footer', 'raythompsonwebdev_google_script');
 ...
 * 
 */
function raythompsonwebdev_cc_mime_types($mimes)
{
    $mimes['svg'] = 'image/svg+xml';
    return $mimes;
}
add_filter('upload_mimes', 'raythompsonwebdev_cc_mime_types');
 ...
 * Sidebar!
 */
function raythompsonwebdev_widgets_init()
{
    register_sidebar(
        array(
...
add_action('widgets_init', 'raythompsonwebdev_widgets_init');
 ...
 */
function raythompsonwebdev_post_thumbnail_sizes_attr($attr, $attachment, $size)
{
 ...
 *
 */
function raythompsonwebdev_content_image_sizes_attr($sizes, $size)
{
